...
namespace PizzeriaWebApp.Controllers
{
	...
	public class AccountController : Controller
	{
		...
		private void CheckForUserAndRoles(string id)
		{
			//prendo l id e inserisco i ruoli grazie al dbcontext
			using (var ctx= new ApplicationDbContext()) {
				...
				if (!ctx.Roles.Any())
				{
					...
					ctx.Roles.Add(new IdentityRole { Name = "operator" });
					ctx.SaveChanges();
				}
				//se ho un utente registrato, lo metto admin
				if (ctx.Users.Count()==1)
				{
					UserManager.AddToRole(id, "admin");
				}
				//potrei fare che metto i primi 2 admin e gli altri x operator
				//devo mettere admin,un utente che corrisponde all'email che fornisco come stringa
		...
		public ActionResult ToggleRole(string id, string roleName)
		{
